...
class LeastLoaded:
	...
	def schedule_dummy_print(self, job_id, task_type, worker_id) -> None:
		print(worker_id, '\tjob_id', job_id, '\t', task_type)
		# self.master.slots_free[worker_id] -= 1
		# if task_type == 'map_task':
		# 	self.master.jobs[job_id]['completed_map_tasks'] += 1
		# 	self.master.slots_free[worker_id] += 1
		# else:
	...
	def schedule_tasks(self) -> None:
		...
		while not self.is_buffer_empty():
			...
			# 			self.schedule_dummy_print(job, 'map_task', worker_id)
			...
			# 				self.schedule_dummy_print(job, 'reduce_task', worker_id)
